terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws/cnapp-staging/eu-central-1/rds/unified-agent-staging-euc1-1.tfstate"     
    region  = "us-east-2"                                                                  
  }
}
...
locals {
  ...
  rds_name         = "unified-agent-staging-euc1-1"
}
...
module "rds" {
  ...
  secret_path       = "secret/eticcprod/infra/aurora-pg/eu-central-1/cnapp-staging/unified-agent-staging-euc1-1"
