module "rds" {
  ...
  secret_path       = "secret/eticcprod/infra/aurora-pg/us-east-2/motific-staging/motf-staging-use2-1"
  db_allowed_cidrs  = [data.aws_vpc.cluster_vpc.cidr_block]
  db_engine_version = "15"
}

resource "aws_rds_global_cluster" "motf_staging_global" {
  global_cluster_identifier = "motf-staging-global"
...
module "rds_secondary" {
  ...
  database_name             = null
  global_cluster_identifier = aws_rds_global_cluster.motf_staging_global.id
  ...
  cluster_name        = "motf-staging-usw2-1"
  kms_key_id          = aws_kms_key.rds_multi_region_secondary.arn
  secret_path         = "secret/staging/infra/aurora-pg/us-west-2/motific-staging/motf-staging-usw2-1"
  db_engine_version   = "15"
  db_allowed_cidrs    = [data.aws_vpc.cluster_vpc.cidr_block]
