...
variable "venture_name" {
  type    = string
  default = "motific-outshift-users" # <-- requires updating
}
...
provider "vault" {
  address   = "https://keeper.cisco.com"
  namespace = "eticloud/apps/motific-outshift-users" # <-- requires updating
  alias     = "venture"
}
...
resource "vault_jwt_auth_backend_role" "admin" {
  depends_on                   = [ vault_policy.admin, vault_policy.user]
  provider                     = vault.venture
  role_name                    = "admin"
  role_type                    = "oidc"
  ...
  bound_claims = {
    memberof = "CN=motific-outshift-users-vault-admin,OU=Cisco Groups,DC=cisco,DC=com"
  }
