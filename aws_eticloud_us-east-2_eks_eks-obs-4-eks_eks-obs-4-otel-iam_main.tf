...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "eks-obs-4-eks-amp-access-management"
...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/aws-eks-obs-4/eks/eks-obs-4-otel-iam.tfstate" # The path should match the pattern terraform_state/<service>/<region>/<name>.tfstate
...
variable "cluster_name" {
  ...
  type        = string
  default     = "eks-obs-4" # The name of the associated EKS cluster. Must be updated.
}

variable "cluster_address" {
  description = "EKS cluster API server endpoint"
  type        = string
  default     = "https://D29D06646B054693B3C7C0B0417E73BF.gr7.us-east-2.eks.amazonaws.com"
}
