terraform {
  backend "s3" {
    ...
    key    = "terraform-state/msk/us-east-2/msk-ostinato-prod-1.tfstate"   # note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
