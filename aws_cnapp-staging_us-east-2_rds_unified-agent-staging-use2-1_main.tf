terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws/cnapp-staging/us-east-2/rds/unified-agent-staging-use2-1.tfstate"     
    region  = "us-east-2"                                                                  
  }
}
...
locals {
  ...
  rds_name         = "unified-agent-staging-use2-1"
}
...
module "rds" {
  ...
  secret_path       = "secret/eticcprod/infra/aurora-pg/us-east-2/cnapp-staging/unified-agent-staging-use2-1"
