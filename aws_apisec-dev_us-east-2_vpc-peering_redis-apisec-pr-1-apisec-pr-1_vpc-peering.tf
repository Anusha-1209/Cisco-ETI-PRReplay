...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc-peering/us-east-2/redis-apisec-pr-1-apisec-pr-1/redis-apisec-pr-1-apisec-pr-1.tfstate"
    region = "us-east-2"
  }
}

locals {
  acceptor_vpc_name  = "redis-apisec-pr-1-vpc"
  requestor_vpc_name = "apisec-pr-1-vpc"
}
