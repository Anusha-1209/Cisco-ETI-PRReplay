terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc/us-east-2/msk-marvin-staging-1-vpc.tfstate"
    region = "us-east-2"
  }
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "msk-marvin-staging-1-vpc"
...
module "vpc" {
  ...
  vpc_name                        = "msk-marvin-staging-1"
  cluster_name                    = "marvin-staging-1"
