...
terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws-apisec-prod/eks/eu-central-1/apisec-prod-eu-1-eks-custom-networking.tfstate"
    region  = "us-east-2"
  }
  required_providers {
    tls = {
...
data "vault_generic_secret" "cluster_certificate" {
  path = "secret/infra/eks/apisec-prod-eu-1/certificate"
}

data "aws_eks_cluster" "cluster" {
  name = "apisec-prod-eu-1"
}
...
data "aws_subnets" "secondary" {
  ...
  filter {
    name   = "tag:Name"
    values = ["apisec-prod-eu-1-vpc-secondary-private*"]
  }
}
