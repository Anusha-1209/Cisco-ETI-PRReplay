terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc/us-east-2/common-staging-use2-vpc-data.tfstate"
    region = "us-east-2"
  }
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "common-staging-use2-vpc-data-vpc"
...
module "vpc" {
  ...
  vpc_name                        = "common-staging-use2-vpc-data"
  cluster_name                    = "common-staging-use2-vpc-data"
