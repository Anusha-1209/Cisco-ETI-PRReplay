...
terraform {
  backend "s3" {
    ...
    key           = "terraform-state/motific-preview/us-east-2/s3/motific-preview-sagemaker.tfstate" # #note the path here. It should match the pattern terraform_state/<service>/<region>/<name>.tfstate
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "Motific SageMaker"
...
module "s3" {
  ...
  bucket_name = "motific-preview-sagemaker"
  CSBApplicationName    = "Motific SageMaker"
