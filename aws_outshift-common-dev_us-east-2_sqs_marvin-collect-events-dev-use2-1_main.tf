...
resource "aws_cloudwatch_metric_alarm" "marvin-collect-events-dlq-alarm-dev-use2-1" {
  alarm_name          = "${aws_sqs_queue.marvin-collect-events-dlq-dev-use2-1.name}-not-empty-alarm"
  alarm_description   = "Items are on the ${aws_sqs_queue.marvin-collect-events-dlq-dev-use2-1.name} queue"
  ...
  alarm_actions       = [aws_sns_topic.marvin-collect-events-dlq-sns-alarm-dev-use2-1.arn]
  tags = {
    CSBDataClassification = "Cisco Restricted"
    CSBEnvironment        = "NonProd"
    CSBApplicationName    = "Marvin"
  ...
  dimensions = {
    "QueueName" = aws_sqs_queue.marvin-collect-events-dlq-dev-use2-1.name
  }
}
