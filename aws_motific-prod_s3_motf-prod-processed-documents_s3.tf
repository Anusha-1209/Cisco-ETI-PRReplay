...
terraform {
  backend "s3" {
    ...
    key = "terraform-state/aws/motific-prod/us-east-2/s3/motf-prod-processed-documents.tfstate" # UPDATE ME.
...
module "s3" {
  ...
  bucket_name = "motf-prod-processed-documents" # The name of the S3 bucket.  S3 bucket names are required to be globally unique across all of AWS.
...
resource "aws_s3_bucket_lifecycle_configuration" "motf-prod-processed-documents" {
  bucket = "motf-prod-processed-documents"
  rule {
    id     = "TTL-policy"
    status = "Enabled"
