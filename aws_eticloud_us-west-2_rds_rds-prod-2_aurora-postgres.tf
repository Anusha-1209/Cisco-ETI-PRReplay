...
terraform {
  backend "s3" {
    ...
    key           = "terraform-state/aurora-postgres/us-west-2/rds-prod-2-rds.tfstate"  #note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
module "rds" {
    ...
    vpc_name = "prod-db-vpc-2"           # the name of the VPC where the database instances and clusters will be created.
    cluster_name = "rds-prod-2"              # the name of the database instance
    database_name = "prod02"
    ...
    CSBApplicationName    = "rds-prod-2-db"
