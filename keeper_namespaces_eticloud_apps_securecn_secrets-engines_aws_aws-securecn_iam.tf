data "aws_caller_identity" "current" {
  provider = aws.securecn
}

locals {
    account_id = data.aws_caller_identity.current.account_id
}

#region Create the IAM user secret engine will use to auth against AWS
resource "aws_iam_user" "vault-secret-engine-user-securecn" {
  provider = aws.securecn
  name     = "vault-secret-engine-user-securecn"
}

resource "aws_iam_access_key" "vault-secret-engine-user-securecn" {
  provider = aws.securecn
  user     = aws_iam_user.vault-secret-engine-user-securecn.name
}

resource "aws_iam_user_policy" "vault-secret-engine-user-securecn" {
  provider = aws.securecn
  name     = "vault-secret-engine-user-securecn"
  user     = aws_iam_user.vault-secret-engine-user-securecn.name
# You should have a list of all the roles from below that you want to allow this user to assume
  policy   = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Sid": "VisualEditor0",
      "Effect": "Allow",
      "Action": "sts:AssumeRole",
      "Resource": [
        "${aws_iam_role.ci-default-role.arn}",
        "${aws_iam_role.ci-s3-push.arn}"
      ]
    }
  ]
}
EOF
  
}
#endregion

#region ci-s3-push role and policy
resource "aws_iam_role" "ci-s3-push" {
  provider           = aws.securecn
  name               = "ci-s3-push"
  assume_role_policy = jsonencode({
    "Version" : "2012-10-17",
    "Statement" : [
      {
        "Effect" : "Allow",
        "Principal" : {
          "AWS" : "${aws_iam_user.vault-secret-engine-user-securecn.arn}"
        },
        "Action" : "sts:AssumeRole"
      }
    ]
  })

  tags = {
    Name = "ci-s3-push"
  }
}

resource "aws_iam_policy" "ci-s3-write-policy" {
  provider    = aws.securecn
  name        = "ci-s3-write-policy"
  description = "write access to S3 storage"

  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Effect": "Allow",
      "Action": ["s3:ListBucket"],
      "Resource": [
        "arn:aws:s3:::securecn-dev",
        "arn:aws:s3:::vmclarity-release"
      ]
    },
    {
      "Effect": "Allow",
      "Action": [
        "s3:PutObject",
        "s3:GetObject",
        "s3:PutObjectAcl"
      ],
      "Resource": [
        "arn:aws:s3:::securecn-dev/*",
        "arn:aws:s3:::vmclarity-release/*"
      ]
    }
  ]
}
EOF
}

resource "aws_iam_role_policy_attachment" "ci-s3-write-policy-attach" {
  provider   = aws.securecn
  role       = aws_iam_role.ci-s3-push.name
  policy_arn = aws_iam_policy.ci-s3-write-policy.arn
}
#endregion

#region ci-default role and policy attachments
resource "aws_iam_role" "ci-default-role" {
  provider           = aws.securecn
  name               = "ci-default"
  assume_role_policy = jsonencode({
    "Version" : "2012-10-17",
    "Statement" : [
      {
        "Effect" : "Allow",
        "Principal" : {
          "AWS" : "${aws_iam_user.vault-secret-engine-user-securecn.arn}"
        },
        "Action" : "sts:AssumeRole"
      },
      {
        "Effect": "Allow",
        "Principal": {
          "AWS": "arn:aws:iam::${local.account_id}:root"
        },
        "Action": "sts:AssumeRole",
        "Condition": {}
    }
    ]
  })

  tags = {
    Name = "ci-default"
  }
}

resource "aws_iam_role_policy_attachment" "ci-default-s3-write-policy-attach" {
  provider   = aws.securecn
  role       = aws_iam_role.ci-default-role.name
  policy_arn = aws_iam_policy.ci-s3-write-policy.arn
}
#endregion
