...
terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws-apisec-staging/eks/us-east-2/apisec-staging-1-eks-custom-networking.tfstate"
    region  = "us-east-2"
  }
  required_providers {
    tls = {
...
data "aws_subnets" "secondary" {
  ...
  filter {
    name   = "tag:Name"
    values = ["apisec-staging-1-vpc-secondary-private*"]
  }
}
