...
terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws-apisec-prod/eks/us-east-2/apisec-prod-1-eks-cm.tfstate"  # Note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
data "vault_generic_secret" "cluster_certificate" {
    path = "secret/infra/eks/apisec-prod-1/certificate"
}

data "vault_generic_secret" "aws_auth_configmap"{
    path = "secret/infra/eks/apisec-prod-1/aws-auth"
}
