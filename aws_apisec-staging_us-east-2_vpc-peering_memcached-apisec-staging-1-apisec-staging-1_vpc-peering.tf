...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc-peering/us-east-2/memcached-apisec-staging-1-apisec-staging-1/memcached-apisec-staging-1-apisec-staging-1.tfstate"
    region = "us-east-2"
  }
}

locals {
  acceptor_vpc_name  = "memcached-apisec-staging-1-vpc"
  requestor_vpc_name = "apisec-staging-1-vpc"
}
