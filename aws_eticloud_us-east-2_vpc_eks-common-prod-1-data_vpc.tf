terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc/us-east-2/eks-common-prod-1-data.tfstate"
    region = "us-east-2"
  }
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "eks-common-prod-1-data-vpc"
...
module "vpc" {
  ...
  vpc_name                        = "eks-common-prod-1-data"
  cluster_name                    = "eks-common-prod-1-data"
