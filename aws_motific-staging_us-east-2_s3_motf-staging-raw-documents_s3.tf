...
terraform {
  backend "s3" {
    ...
    key = "terraform-state/aws/motific-staging/us-east-2/s3/motf-staging-raw-documents.tfstate" # UPDATE ME.
...
module "s3" {
  ...
  bucket_name = "motf-staging-raw-documents" # The name of the S3 bucket.  S3 bucket names are required to be globally unique across all of AWS.
