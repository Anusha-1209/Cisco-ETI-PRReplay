...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/s3/eu-north-1/argocd-cnapp-backup.tfstate" #note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
variable "AWS_INFRA_REGION" {
  ...
  default     = "eu-north-1" #Set the region for the resources to be created.
}
# Infra AWS Provider
provider "aws" {
...
module "s3" {
  ...
  bucket_name = "argocd-cnapp-backup" # The name of the S3 bucket.  S3 bucket names are required to be globally unique across all of AWS.
