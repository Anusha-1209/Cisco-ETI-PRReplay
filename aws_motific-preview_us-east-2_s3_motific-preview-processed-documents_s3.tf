...
terraform {
  backend "s3" {
    ...
    key = "terraform-state/aws/motific-preview/us-east-2/s3/motific-preview-processed-documents.tfstate" # UPDATE ME.
...
module "s3" {
  ...
  bucket_name = "motific-preview-processed-documents" # The name of the S3 bucket.  S3 bucket names are required to be globally unique across all of AWS.
  ...
  CSBApplicationName    = "Motific"
  CSBResourceOwner      = "Outshift SRE"
  CSBCiscoMailAlias     = "eti-sre@cisco.com"
  CSBDataTaxonomy       = "Cisco Operations Data"
}
resource "aws_s3_bucket_lifecycle_configuration" "motific-preview-processed-documents" {
  bucket = "motific-preview-processed-documents"
  rule {
    id     = "TTL-policy"
    status = "Enabled"
