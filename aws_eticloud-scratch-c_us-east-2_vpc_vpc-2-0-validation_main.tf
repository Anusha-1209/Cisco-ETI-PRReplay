...
terraform {
  backend "s3" {
    ...
    key = "terraform-state/eticloud-scratch-c/us-east-2/vpc/vpc-2-0-validation.tfstate"
...
locals {
  name             = "vpc-2-0-validation"
  region           = "us-east-2"
  aws_account_name = "eticloud-scratch-c"
}

module "vpc" {
  source           = "git::https://github.com/cisco-eti/sre-tf-module-vpc-allinone.git?ref=sre-vpc-v2"
