terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws/eticloud/global/rds/global-rds-iam-dev-1.tfstate"     
    region  = "us-east-2"                                                                  
  }
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "global-rds-iam-dev-1_secondary"
      CiscoMailAlias     = "eti-sre-admins@cisco.com"
      DataClassification = "Cisco Confidential"
      DataTaxonomy       = "Cisco Operations Data"
      EnvironmentName    = "NonProd"
...
module "rds_secondary" {
  ...
  instances                 = {}
  cluster_name              = "global-rds-iam-dev-usw2-1"
