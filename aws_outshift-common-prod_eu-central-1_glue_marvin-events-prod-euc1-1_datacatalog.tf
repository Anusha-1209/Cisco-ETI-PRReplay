...
resource "aws_security_group" "marvin-prod-euc1-1-data-glue" {
  name        = "marvin-prod-euc1-data-glue"
...
resource "aws_vpc_security_group_ingress_rule" "allow_tcp_ipv4" {
  security_group_id            = aws_security_group.marvin-prod-euc1-1-data-glue.id
  referenced_security_group_id = aws_security_group.marvin-prod-euc1-1-data-glue.id
  from_port                    = 0
  ip_protocol                  = "tcp"
  to_port                      = 65535
}
...
resource "aws_glue_connection" "rds-marvin-connection" {
  ...
  connection_properties = {
    JDBC_CONNECTION_URL  = "jdbc:postgresql://${data.aws_rds_cluster.marvin-prod-euc1-1.endpoint}:5432/marvin"
  ...
  physical_connection_requirements {
    ...
    security_group_id_list = [aws_security_group.marvin-prod-euc1-1-data-glue.id]
