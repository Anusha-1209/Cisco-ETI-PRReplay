...
terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws-apisec-prod/eks/us-east-2/apisec-prod-1-eks-custom-networking.tfstate"
    region  = "us-east-2"
  }
  required_providers {
    tls = {
...
data "aws_subnets" "secondary" {
  ...
  filter {
    name   = "tag:Name"
    values = ["apisec-prod-1-vpc-secondary-private*"]
  }
}
