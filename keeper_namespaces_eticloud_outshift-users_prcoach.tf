# Outshift prcoach Vault Role
resource "vault_jwt_auth_backend_role" "prcoach" {
  depends_on = [vault_policy.prcoach]
  ...
  bound_claims = {
    memberof = "CN=outshift-vault-prcoach,OU=Cisco Groups,DC=cisco,DC=com"
  }
