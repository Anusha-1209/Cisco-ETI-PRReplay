...
terraform {
  backend "s3" {
    ...
    key           = "terraform-state/aurora-postgres/us-east-2/rds-staging-1-rds.tfstate"  #note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
module "rds" {
    ...
    vpc_name = "nonprod-db-vpc-1"           # the name of the VPC where the database instances and clusters will be created.
    cluster_name = "rds-staging-1"              # the name of the database instance
    database_name = "staging01"
    ...
    CSBApplicationName    = "rds-staging-1-db"
