...
resource "aws_iam_policy" "aws_karpenter_controller_policy" {
  name        = "KarpenterControllerPolicy-${var.eks_name}"
  description = "${var.eks_name} AWS Karpenter Controller Role IAM Policy"
  policy = jsonencode({
    "Version": "2012-10-17",
    "Statement": [
        {
        ...
        },
        {
            "Effect": "Allow",
            "Action": "iam:PassRole",
            "Resource": "arn:aws:iam::${local.account_id}:role/KarpenterNodeRole-${var.eks_name}",
            "Sid": "PassNodeIAMRole"
        },
        {
            "Effect": "Allow",
            "Action": "eks:DescribeCluster",
            "Resource": "arn:aws:eks:${var.aws_default_region}:${local.account_id}:cluster/${var.eks_name}",
        ...
        },
        {
            ...
            "Condition": {
            "StringEquals": {
                "aws:RequestTag/kubernetes.io/cluster/${var.eks_name}": "owned",
                "aws:RequestTag/topology.kubernetes.io/region": "${var.aws_default_region}"
            },
            "StringLike": {
        ...
        },
        {
            ...
            "Condition": {
            "StringEquals": {
                "aws:ResourceTag/kubernetes.io/cluster/${var.eks_name}": "owned",
                "aws:ResourceTag/topology.kubernetes.io/region": "${var.aws_default_region}",
                "aws:RequestTag/kubernetes.io/cluster/${var.eks_name}": "owned",
                "aws:RequestTag/topology.kubernetes.io/region": "${var.aws_default_region}"
            },
            "StringLike": {
...
resource "aws_iam_role" "aws_karpenter_controller_role" {
  name = "KarpenterControllerRole-${var.eks_name}"
