terraform {
  backend "s3" {
    ...
    key    = "terraform-state/aws-apisec-prod/elasticache/us-east-2/redis-apisec-prod-1.tfstate"
    region = "us-east-2"
  }
}
...
locals {
  ...
  subnet_group_name    = "redis-apisec-prod-1-vpc-ec-subnet-group"
  elasticache_name     = "redis-apisec-prod-1"
  redis_vpc_name       = "redis-apisec-prod-1-vpc"
  eks_cluster_vpc_name = "apisec-prod-1-vpc"
}
