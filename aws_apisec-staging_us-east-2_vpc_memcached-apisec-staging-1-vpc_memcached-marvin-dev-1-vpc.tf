terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc/us-east-2/memcached-apisec-staging-1--vpc.tfstate"
    region = "us-east-2"
  }
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "memcached-apisec-staging-1-vpc"
...
module "vpc" {
  ...
  vpc_name                        = "memcached-apisec-staging-1"
  cluster_name                    = "apisec-staging-1"
