...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/aws-eti-ci/vpc/us-east-2/eks-gitops-genai-1-eks-cm.tfstate" # Note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
variable "AWS_INFRA_REGION" {
  ...
  default     = "us-east-2" # The region in which the EKS cluster will be created. Must match the region in which the VPC was created.
}
...
data "vault_generic_secret" "cluster_certificate" {
  path = "secret/infra/eks/eks-gitops-genai-1/certificate"
}

data "vault_generic_secret" "aws_auth_configmap" {
  path = "secret/infra/eks/eks-gitops-genai-1/aws-auth"
}

data "aws_eks_cluster" "cluster" {
  name = "eks-gitops-genai-1"
}
