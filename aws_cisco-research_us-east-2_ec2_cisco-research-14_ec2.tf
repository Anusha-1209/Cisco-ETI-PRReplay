...
data "vault_generic_secret" "aws_infra_credential" {
  ...
  path     = "secret/infra/aws/cisco-research/terraform_admin" # Defines which account the resources will be created in. Can be eticloud, scratch, eticloud-scratch-c, eticloud-cil, eticloud-demo
}
...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/ec2/us-east-2/cisco-research-14.tfstate" # Note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
module "ec2" {
  ...
  ec2_name                        = "cisco-research-14" # The name of the instance(s) to be created. Will be appended with a number.
  ...
  vpc_name                        = "cisco-research-vpc" # The name of the VPC where the instance will be created.
  instance_os                     = "Ubuntu20"   # Should usually be Ubuntu20 or AmazonLinux2; will always Amazon Linux 2 if gpu_enabled set to true.
  ec2_volume_size                 = "500"           # The size, in GB, of the root volume. Default is 100GB
  region                          = "us-east-2"
  ...
  tag_application_name            = "cisco-research-14"
