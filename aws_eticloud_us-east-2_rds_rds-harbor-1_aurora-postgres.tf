...
terraform {
  backend "s3" {
    ...
    key           = "terraform-state/aurora-postgres/us-east-2/rds-harbor-1-rds.tfstate"  #note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
module "rds" {
    source = "git::https://wwwin-github.cisco.com/eti/sre-tf-module-aws-aurora-postgres?ref=1.0.0" # The reference specifies the version of the module.
    vpc_name = "eks-prod-1"           # the name of the VPC where the database instances and clusters will be created.
    cluster_name = "harbor01"              # the name of the database instance
    database_name = "harbor01"
    ...
    CSBApplicationName    = "harbor-1-db"
