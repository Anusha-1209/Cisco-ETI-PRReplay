...
terraform {
  backend "s3" {
    ...
    key = "terraform-state/aws/outshift-common-prod/us-east-2/s3/marvin-collect-events-prod-euc1-1.tfstate"
...
resource "aws_sqs_queue" "marvin-prod-euc1-1-collect-events" {
  ...
  redrive_policy = jsonencode({
    deadLetterTargetArn = aws_sqs_queue.marvin-collect-events-dlq-prod-euc1-1.arn
    maxReceiveCount     = 4
  })
  tags = {
