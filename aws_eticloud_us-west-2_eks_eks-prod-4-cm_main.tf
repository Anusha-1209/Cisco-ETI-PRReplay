...
terraform {
  backend "s3" {
    ...
    key     = "terraform-state/eks/us-east-2/eks-prod-4-cm.tfstate" #note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
data "vault_generic_secret" "cluster_certificate" {
    path = "secret/infra/eks/eks-prod-4/certificate"
}

data "vault_generic_secret" "aws_auth_configmap"{
    path = "secret/infra/eks/eks-prod-4/aws-auth"
}
