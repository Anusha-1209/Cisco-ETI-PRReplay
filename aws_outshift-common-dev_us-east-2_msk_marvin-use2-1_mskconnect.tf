...
resource "aws_iam_policy" "aws_msk_connect_msk_cluster_policy" {
  name = "MSKConnectMSKClusterPolicy-${aws_msk_cluster.marvin-dev-use2-1-msk.cluster_name}"
  policy = jsonencode({
    "Version"   = "2012-10-17"
    "Statement" = [
      {
        ...
        "Resource" : [
          aws_msk_cluster.marvin-dev-use2-1-msk.arn
        ]
      },
      {
        "Effect" : "Allow",
        ...
        "Resource" : [
          "arn:aws:kafka:us-east-2:${data.aws_caller_identity.current.id}:topic/${aws_msk_cluster.marvin-dev-use2-1-msk.cluster_name}/*"
        ]
      },
      {
        "Effect" : "Allow",
      ...
      },
      {
        ...
        "Resource" : [
          "arn:aws:kafka:us-east-2:${data.aws_caller_identity.current.id}:group/${aws_msk_cluster.marvin-dev-use2-1-msk.cluster_name}/*/__amazon_msk_connect_*",
          "arn:aws:kafka:us-east-2:${data.aws_caller_identity.current.id}:group/${aws_msk_cluster.marvin-dev-use2-1-msk.cluster_name}/*/connect-*"
        ]
      }
    ]
  })
...
resource "aws_iam_policy" "aws_msk_connect_s3_bucket_policy" {
  name = "MSKConnectS3BucketPolicy-${aws_msk_cluster.marvin-dev-use2-1-msk.cluster_name}"
  policy = jsonencode({
    "Version": "2012-10-17",
    "Statement": [
      {
...
resource "aws_iam_role" "aws_msk_connect_role" {
  name               = "MSKConnectRole-${aws_msk_cluster.marvin-dev-use2-1-msk.cluster_name}"
...
import {
  ...
  id = "marvin-dev-use2-1-msk-s3-connectors"
}
...
import {
  ...
  id = "marvin-dev-use2-1-msk-s3-connectors/confluentinc-kafka-connect-s3-10.5.7.zip"
}
...
resource "aws_cloudwatch_log_group" "marvin-msk-connect-log_group" {
  name = "marvin-dev-use2-1-msk-connect-logs"
}

resource "aws_s3_bucket" "marvin-dev-use2-1-msk-sink" {
  bucket = "msk-connect-marvin-dev-use2-1"
}

resource "aws_s3_bucket_lifecycle_configuration" "bucket-config" {
  bucket = aws_s3_bucket.marvin-dev-use2-1-msk-sink.id
...
resource "aws_mskconnect_connector" "marvin-dev-use2-1-msk-connect" {
  name = "${aws_msk_cluster.marvin-dev-use2-1-msk.cluster_name}S3SinkConnect"
  ...
  connector_configuration = {
    ...
    "s3.bucket.name"="msk-connect-marvin-dev-use2-1"
    "timestamp.extractor"="Record"
  }

  kafka_cluster {
    apache_kafka_cluster {
      bootstrap_servers = aws_msk_cluster.marvin-dev-use2-1-msk.bootstrap_brokers_tls

      vpc {
        security_groups = [aws_security_group.marvin-dev-use2-1-msk.id]
