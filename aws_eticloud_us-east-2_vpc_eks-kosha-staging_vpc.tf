...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc/us-east-2/eks-kosha-staging-vpc.tfstate" # #note the path here. It should match the pattern terraform_state/<service>/<region>/<name>.tfstate
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "eks-kosha-staging-vpc"
...
module "vpc" {
  source                          = "git::https://wwwin-github.cisco.com/eti/sre-tf-module-aws-vpc?ref=SRE-5290" # The reference specifies the version of the 
  vpc_cidr                        = "10.43.0.0/16"                                                            # You should use a /16 RFC1918 CIDR. Subnets will be automatically calculated if not specificed.
  region                          = var.AWS_INFRA_REGION
  vpc_name                        = "eks-kosha-staging"                                                       # 
  cluster_name                    = "eks-kosha-staging"                                                       # The parameter sets tags on subnets for EKS to consume. Please contact the SRE team if you have need more than one EKS cluster in your VPC.
