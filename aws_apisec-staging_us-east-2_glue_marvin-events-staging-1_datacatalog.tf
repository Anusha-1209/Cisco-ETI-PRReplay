terraform {
  backend "s3" {
    ...
    key    = "terraform-state/aws-apisec-staging/eks/us-east-2/glue-events-table-marvin-staging-1.tfstate"
    region = "us-east-2"
  }
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "glue-events-table-marvin-staging-1"
...
resource "aws_glue_catalog_table" "aws_glue_catalog_marvin_table" {
  ...
  storage_descriptor {
    location      = "s3://msk-connect-marvin-staging-1/topics/events/"
