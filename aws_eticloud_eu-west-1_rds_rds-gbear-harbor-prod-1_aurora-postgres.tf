...
terraform {
  backend "s3" {
    ...
    key           = "terraform-state/aurora-postgres/eu-west-1/rds-gbear-harbor-prod-1.tfstate"  #note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
module "rds" {
    ...
    vpc_name = "greatbear-eu-west-1-vpc"           # the name of the VPC where the database instances and clusters will be created.
    cluster_name = "rds-gbear-harbor-prod-1"              # the name of the database instance
    ...
    CSBApplicationName    = "greatbear-harbor-1-db"
