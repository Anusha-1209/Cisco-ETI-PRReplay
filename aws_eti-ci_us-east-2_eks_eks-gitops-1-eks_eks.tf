terraform {
  backend "s3" {
    ...
    key    = "terraform-state/aws-eti-ci/eks/us-east-2/eks-gitops-1-eks.tfstate"
    region = "us-east-2"
  }
}
...
module "eks" {
  ...
  aws_auth_additional_roles = [  
    ...
    {
      rolearn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/KarpenterNodeRole-eks-gitops-1",
