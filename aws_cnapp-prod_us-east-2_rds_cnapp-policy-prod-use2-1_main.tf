terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws/cnapp-prod/us-east-2/rds/cnapp-policy-prod-rds-use2-1.tfstate"     
    region  = "us-east-2"                                                                  
  }
}
...
locals {
  ...
  data_vpc         = "cnapp-prod-use2-data"
  eks_vpc          = "cnapp-prod-use2-1"
  rds_name         = "policy-prod-rds-use2-1"
}
...
module "rds" {
  ...
  secret_path       = "secret/eticcprod/infra/aurora-pg/us-east-2/cnapp-prod/policy-prod-rds-use2-1"
