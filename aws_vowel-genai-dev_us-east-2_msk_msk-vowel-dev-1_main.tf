...
data "aws_security_group" "eks-sg" {
  ...
  filter {
    name = "tag:Name"
    values = ["eks-cluster-sg-vowel-dev-1*"]
  }
}
...
resource "aws_secretsmanager_secret" "msk_secret" {
  name       = "AmazonMSK_msk-vowel-dev"
  kms_key_id = aws_kms_key.msk_custom_key.key_id
  recovery_window_in_days = 0

  description = "SASL/SCRAM secret for msk-vowel-dev"
}
...
data "vault_generic_secret" "msk_credentials" {
  ...
  path = "secret/infra/msk/msk-vowel-dev-1"
}
