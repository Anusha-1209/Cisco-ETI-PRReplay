terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws/cnapp-prod/eu-central-1/rds/cnapp-policy-prod-euc1-1.tfstate"     
    region  = "us-east-2"                                                                  
  }
}
...
locals {
  ...
  data_vpc         = "cnapp-prod-euc1-data"
  eks_vpc          = "cnapp-prod-euc1-1"
  rds_name         = "policy-prod-rds-euc1-1"
}
...
module "rds" {
  ...
  secret_path       = "secret/eticcprod/infra/aurora-pg/eu-central-1/cnapp-prod/cnapp-policy-prod-euc1-1"
