terraform {
  backend "s3" {
    ...
    key    = "terraform-state/msk/us-east-2/motf-e2e-use2-1-msk.tfstate" # note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
