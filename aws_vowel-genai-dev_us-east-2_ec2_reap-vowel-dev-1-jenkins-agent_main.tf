...
terraform {
  backend "s3" {
    ...
    key = "terraform-state/vowel-genai-dev/us-east-2/ec2/reap-vowel-dev-1-jenkins-agent.tfstate"
...
module "ec2" {
  ...
  # EC2 Instance name
  name = "reap-vowel-dev-1-jenkins-agent"
  ...
  environment         = "NonProd"
  resource_owner      = "mmarvit@cisco.com"
}