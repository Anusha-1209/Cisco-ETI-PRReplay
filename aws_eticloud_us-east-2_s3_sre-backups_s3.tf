terraform {
  ...
  backend "s3" {
    ...
    key            = "terraform-state/s3/us-east-2/cisco-eti-sre-backups.tfstate"
    region         = "us-east-2"
...
provider "aws" {
  ...
  default_tags {
    tags = {
      CiscoMailAlias = "eti-sre-admins_at_cisco_dot_com"
...
resource "aws_s3_bucket" "b" {
  bucket = "cisco-eti-sre-backups"
  acl    = "private"
  versioning {
    enabled = true
  }
  lifecycle_rule {
    id      = "expires-in-10-days"
    enabled = true
    prefix = "/ldap"
    tags = {
      autoclean = "true"
