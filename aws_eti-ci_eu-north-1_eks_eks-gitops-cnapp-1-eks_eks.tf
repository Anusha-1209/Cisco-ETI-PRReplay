...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/aws-eti-ci/vpc/eu-north-1/eks-gitops-cnapp-1-eks.tfstate" # The path should match the pattern terraform_state/<service>/<region>/<name>.tfstate
...
variable "AWS_INFRA_REGION" {
  ...
  default     = "eu-north-1" # The region in which the EKS cluster will be created. 
...
module "eks" {
  ...
  cluster_name    = "eks-gitops-cnapp-1" # The name of the cluster.
  ...
  # ami_date = "2024-03-01"
  vpc_name   = "eks-gitops-cnapp-1-vpc" # The name of the VPC in which the cluster should be created.
