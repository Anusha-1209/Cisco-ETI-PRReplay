...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/s3/us-east-2/outshift-headless-cms-s3.tfstate" #note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
module "s3" {
  ...
  bucket_name = "outshift-headless-cms-s3" # The name of the S3 bucket.  S3 bucket names are required to be globally unique across all of AWS.
  ...
  CSBEnvironment        = "Prod"
  CSBApplicationName    = "outshift-website"
...
# resource "aws_s3_bucket_cors_configuration" "outshift-headless-cms-s3" {
#   bucket = data.aws_s3_bucket.bucket.id
#   cors_rule {
#     allowed_methods = ["GET", "PUT", "POST", "DELETE"]
#     allowed_origins = ["https://outshift-admin-panel.dev.eticloud.io", "https://outshift-admin-panel.staging.eticloud.io", "https://outshift-admin-panel.eticloud.io"]
#     allowed_headers = ["*"]
