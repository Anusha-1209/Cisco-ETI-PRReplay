...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc-peering/us-east-2/data-motf-staging-use2-1.tfstate"
    region = "us-east-2"
  }
}

locals {
  acceptor_vpc_name  = "motf-staging-use2-1"
  requestor_vpc_name = "motf-staging-use2-data"
}
