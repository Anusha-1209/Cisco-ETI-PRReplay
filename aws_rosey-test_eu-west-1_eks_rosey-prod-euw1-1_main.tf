terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws/rosey-prod/eu-west-1/eks/rosey-prod-euw1-1.tfstate"       # UPDATE ME.
...
locals {
  name = "rosey-prod-euw1-1"
  region = "eu-west-1"
  aws_account_name = "rosey-test"
}

module "eks_all_in_one" {
  source            = "git::https://github.com/cisco-eti/sre-tf-module-eks-allinone.git?ref=extend_ami" # based on v0.1.0

  name              = local.name              # EKS cluster name
  region            = local.region            # AWS provider region
  aws_account_name  = local.aws_account_name  # AWS account name
  cidr              = "10.9.0.0/16"           # VPC CIDR
