...
variable "venture_name" {
  type    = string
  default = "vowel" # <-- requires updating
}
...
provider "vault" {
  address   = "https://keeper.cisco.com"
  namespace = "eticloud/apps/vowel" # <-- requires updating
  alias     = "venture"
}
...
resource "vault_jwt_auth_backend_role" "developer-staging" {
  depends_on                   = [ vault_policy.policy["developer-staging"], vault_policy.policy["developer-staging"] ]
  provider                     = vault.venture
  role_name                    = "developer-staging"
  role_type                    = "oidc"
  backend                      = vault_jwt_auth_backend.oidc.path
  allowed_redirect_uris        = ["https://keeper.cisco.com/ui/vault/auth/oidc/oidc/callback",
                                  "https://east.keeper.cisco.com/ui/vault/auth/oidc/oidc/callback",
  ...
  bound_audiences              = [var.oidc_client_id]
  bound_claims = {
    memberof = "CN=eti-${var.venture_name}-vault-developer-staging,OU=Cisco Groups,DC=cisco,DC=com"
  }
  disable_bound_claims_parsing = true
  bound_claims_type            = "string"
  claim_mappings = {
  ...
  oidc_scopes                  = ["profile", "email", "openid"]
  user_claim                   = "sub"
  token_policies               = [vault_policy.policy["developer-staging"].name,
                                  vault_policy.default.name]
}

resource "vault_jwt_auth_backend_role" "developer-prod" {
  depends_on                   = [ vault_policy.policy["developer-prod"], vault_policy.policy["developer-prod"] ]
  provider                     = vault.venture
  role_name                    = "developer-prod"
  role_type                    = "oidc"
  backend                      = vault_jwt_auth_backend.oidc.path
  allowed_redirect_uris        = ["https://keeper.cisco.com/ui/vault/auth/oidc/oidc/callback",
                                  "https://east.keeper.cisco.com/ui/vault/auth/oidc/oidc/callback",
  ...
  bound_audiences              = [var.oidc_client_id]
  bound_claims = {
    memberof = "CN=eti-${var.venture_name}-vault-developer-prod,OU=Cisco Groups,DC=cisco,DC=com"
  }
  disable_bound_claims_parsing = true
  bound_claims_type            = "string"
  claim_mappings = {
  ...
  oidc_scopes                  = ["profile", "email", "openid"]
  user_claim                   = "sub"
  token_policies               = [vault_policy.policy["developer-prod"].name,
                                  vault_policy.default.name]
}
...
locals {
  policies = {
    ...
    "external-secrets-preview" = "policies/external-secrets-preview.hcl",
    "external-secrets-prod"    = "policies/external-secrets-prod.hcl",
    "external-secrets-staging" = "policies/external-secrets-staging.hcl",
    "vowel-admin"              = "policies/vowel-admin.hcl",
    "vowel-dev-user"           = "policies/vowel-dev-user.hcl",
    "vowel-perf-user"          = "policies/vowel-perf-user.hcl",
    "vowel-prod-user"          = "policies/vowel-prod-user.hcl",
    "vowel-staging-user"       = "policies/vowel-staging-user.hcl",
  }
}
