...
locals {
  ...
  len_intra_subnets       = max(length(local.intra_subnets), length(var.intra_subnet_ipv6_prefixes))
...
resource "aws_subnet" "intra" {
  ...
  availability_zone                              = length(regexall("^[a-z]{2}-", element(local.azs, count.index))) > 0 ? element(local.azs, count.index) : null
  availability_zone_id                           = length(regexall("^[a-z]{2}-", element(local.azs, count.index))) == 0 ? element(local.azs, count.index) : null
  cidr_block                                     = var.intra_subnet_ipv6_native ? null : element(concat(local.intra_subnets, [""]), count.index)
  ...
  tags = merge(
    {
      Name = try(
        ...
        format("${var.name}-${var.intra_subnet_suffix}-%s", element(local.azs, count.index))
      )
    },
    var.tags,
