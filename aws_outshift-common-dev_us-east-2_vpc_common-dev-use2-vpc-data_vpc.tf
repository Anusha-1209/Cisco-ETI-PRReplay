terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc/us-east-2/common-dev-use2-vpc-data.tfstate"
    region = "us-east-2"
  }
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "common-dev-use2-vpc-data-vpc"
...
module "vpc" {
  ...
  region                          = "us-east-2"
  vpc_cidr                        = "10.22.0.0/22"
  vpc_name                        = "common-dev-use2-vpc-data"
  cluster_name                    = "common-dev-use2-vpc-data"
