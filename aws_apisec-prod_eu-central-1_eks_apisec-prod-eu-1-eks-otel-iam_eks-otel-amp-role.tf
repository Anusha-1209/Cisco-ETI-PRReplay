...
terraform {
  backend "s3" {
    ...
    # The buckets are eticloud-tf-state-sandbox, eticloud-tf-state-prod, eticloud-tf-state-prod. 
    # <bucket_name> in eticloud-tf-state-<bucket_name> should match the Environment tag below.
    key    = "terraform-state/aws-apisec-prod/iam/apisec-prod-eu-1-eks-otel-amp-role.tfstate" # The path should match the pattern terraform_state/<service>/<region>/<name>.tfstate
...
variable "cluster_name" {
  ...
  type    = string
  default = "apisec-prod-eu-1" # The name of the associated EKS cluster. Must be updated.
}

variable "cluster_region" {
  description = "EKS cluster region"
  type    = string
  default = "eu-central-1" # The name of the associated EKS cluster. Must be updated.
}
