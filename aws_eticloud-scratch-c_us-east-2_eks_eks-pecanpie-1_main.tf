terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws/rosey-test/us-east-2/eks/eks-pecanpie-1.tfstate"         # UPDATE ME.
...
module "eks_all_in_one" {
  source            = "git::https://github.com/cisco-eti/sre-tf-module-eks-allinone.git?ref=latest" # Based on v0.1.1

  name              = local.name              # EKS cluster name
  region            = local.region            # AWS provider region
  aws_account_name  = local.aws_account_name  # AWS account name
  cidr              = "10.79.0.0/16"           # VPC CIDR
