...
data "aws_security_group" "eks_sg" {
  ...
  filter {
    name   = "tag:Name"
    values = ["eks-cluster-sg-motf-e2e-use2-1-*"]
  }
}
...
resource "aws_secretsmanager_secret" "msk_secret" {
  name                    = "AmazonMSK_motific-e2e"
  kms_key_id              = aws_kms_key.msk_custom_key.key_id
  recovery_window_in_days = 0

  description = "SASL/SCRAM secret for msk_motf-e2e-use2-1"
}
...
data "vault_generic_secret" "msk_credentials" {
  ...
  path     = "secret/infra/msk/motf-e2e-use2-1"
}
...
resource "aws_security_group_rule" "kafka_ingress_eks_motf-e2e-use2-1" {
  type              = "ingress"
  from_port         = 9096
  to_port           = 9096
  protocol          = "tcp"
