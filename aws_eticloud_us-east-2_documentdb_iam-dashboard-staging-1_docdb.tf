terraform {
  backend "s3" {
    ...
    key    = "terraform-state/docdb/us-east-2/iam-dashboard-staging-1-docdb.tfstate"
    region = "us-east-2"
  }
...
data "vault_generic_secret" "genie_docdb_credential" {
  path     = "secret/staging/iam-dashboard"
...
resource "aws_security_group" "iam_dashboard_staging_docdb" {
  name        = "iam-dashboard-staging-docdb"
  description = "Allow inbound traffic from eks-prod-3 to iam-dashboard staging docdb"
  vpc_id      = data.aws_vpc.db_vpc.id

  tags = {
    "Name" = "iam-dashboard-staging-docdb-sg"
  }
...
resource "aws_docdb_cluster" "docdb" {
  cluster_identifier = "iam-dashboard-staging-1"

  engine                          = "docdb"
  engine_version                  = "5.0.0"
  db_cluster_parameter_group_name = "default.docdb5.0"
  cluster_members = [
    "iam-dashboard-staging-1"
  ]

  vpc_security_group_ids = [
    "sg-080de64878164f5ab", // DB VPC default security group
    aws_security_group.iam_dashboard_staging_docdb.id
  ]
  availability_zones = [
    "us-east-2a",
    "us-east-2b",
  ...
  depends_on = [aws_security_group.iam_dashboard_staging_docdb]
}
