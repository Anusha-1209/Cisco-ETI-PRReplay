...
data "vault_generic_secret" "gcp_k8sec_dev_1_infra_credentials" {
  provider = vault.eticloud
  path     = "secret/infra/gcp/k8sec-dev"
}

# By setting the GCP provider credentials via the data source above, we control in which project the resources get created.
provider "google" {
  credentials = data.vault_generic_secret.gcp_k8sec_dev_1_infra_credentials.data_json
}
