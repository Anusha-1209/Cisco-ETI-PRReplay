...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/s3/us-east-2/argocd-genai-backup.tfstate" #note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
module "s3" {
  ...
  bucket_name = "argocd-genai-backup" # The name of the S3 bucket.  S3 bucket names are required to be globally unique across all of AWS.
