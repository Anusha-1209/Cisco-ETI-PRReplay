...
provider "vault" {
  address   = "https://keeper.cisco.com"
  namespace = "eticloud/apps/scs" # <-- requires updating
  alias     = "venture"
}
...
# Approles
resource "vault_auth_backend" "approle-test" {
  provider = vault.venture
  type = "approle"
...
resource "vault_approle_auth_backend_role" "approle-test" {
  provider       = vault.venture
  backend        = vault_auth_backend.approle-test.path
  role_name      = "test-role"
...
resource "vault_policy" "external-secrets-dev" {
  provider = vault.venture
...
resource "vault_policy" "external-secrets-prod" {
  provider = vault.venture
...
# eticloud policy
resource "vault_policy" "external-secrets-prod-scs" {
  provider = vault
  name     = "external-secrets-prod-scs"
  policy   = <<EOT
    path "secret/data/*" {
      capabilities = ["read", "list"]
    }
...
resource "vault_policy" "external-secrets-dev-scs" {
  provider = vault
  name     = "external-secrets-dev-scs"
  policy   = <<EOT
    path "secret/data/dev/*" {
      capabilities = ["read", "list"]
    }
