...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/aws-apisec-dev/iam/apisec-pr-1-eks-otel-amp-role.tfstate" # The path should match the pattern terraform_state/<service>/<region>/<name>.tfstate
...
variable "cluster_name" {
  ...
  type    = string
  default = "apisec-pr-1" # The name of the associated EKS cluster. Must be updated.
}
