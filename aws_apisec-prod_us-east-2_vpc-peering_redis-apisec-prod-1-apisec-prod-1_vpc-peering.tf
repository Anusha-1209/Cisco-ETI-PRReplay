...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc-peering/us-east-2/redis-apisec-prod-1-apisec-prod-1/redis-apisec-prod-1-apisec-prod-1.tfstate"
    region = "us-east-2"
  }
}

locals {
  acceptor_vpc_name  = "redis-apisec-prod-1-vpc"
  requestor_vpc_name = "apisec-prod-1-vpc"
}
