...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/aws/outshift-common-prod/us-east-2/vpc-peering/pi-prod-use2-1-data-peering.tfstate" 
    region = "us-east-2"
  }
}

locals {
  acceptor_vpc_name  = "pi-prod-use2-1"
  requestor_vpc_name = "pi-prod-use2-data"
}
