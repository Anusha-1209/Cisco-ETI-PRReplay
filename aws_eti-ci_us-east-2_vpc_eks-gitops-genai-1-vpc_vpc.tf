...
terraform {
  backend "s3" {
    ...
    key           = "terraform-state/aws-eti-ci/vpc/us-east-2/eks-gitops-genai-1-vpc.tfstate" # #note the path here. It should match the pattern terraform_state/<service>/<region>/<name>.tfstate
...
provider "aws" {
  ...
  region      = "us-east-2" # Defines the default region where resources are created if not otherwise specified.
  max_retries = 3
  default_tags {
...
module "vpc" {
  ...
  region                          = "us-east-2" # The region where the resources will be created. Should match the AWS provider region.
  vpc_name                        = "eks-gitops-genai-1" # The vpc_name will get `-vpc` added to it.
  vpc_cidr                        = "10.0.0.0/16" # Check active cloud resources for conflicts.
  cluster_name                    = "eks-gitops-genai-1" # The name of an EKS cluster to be installed.
