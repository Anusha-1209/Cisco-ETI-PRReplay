terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc/us-east-2/motf-prod-use2-data-vpc.tfstate"
    region = "us-east-2"
  }
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "motf-prod-use2-data-vpc"
...
module "vpc" {
  ...
  vpc_name                        = "motf-prod-use2-data"
  cluster_name                    = "motf-prod-use2-data"
