terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc/us-east-2/motf-e2e-use2-data-vpc.tfstate"
    region = "us-east-2"
  }
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "motf-e2e-use2-data-vpc"
...
module "vpc" {
  ...
  vpc_name                        = "motf-e2e-use2-data"
  cluster_name                    = "motf-e2e-use2-data"
