...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/eks/us-east-2/eks-kosha-staging.tfstate" #note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      Environment        = "NonProd"
      ApplicationName    = "eks-kosha-staging-eks"
...
module "eks" {
  source       = "git::https://wwwin-github.cisco.com/eti/sre-tf-module-aws-eks?ref=SRE-5902"
  cluster_name = "eks-kosha-staging" #The name of the cluster.
  #cluster_version and cluster_os combine get the AMI for the worker nodes.
  cluster_version = "1.24"                  #The version of the control plane.
  cluster_os      = "AmazonLinux2"          #Current options are "Ubuntu20" and "AmazonLinux2".
  vpc_name        = "eks-kosha-staging-vpc" #The name of the VPC where the cluster should live.
  ...
  # aws-auth configmap
  # manage_aws_auth_configmap = true # Set to true in SRE module
