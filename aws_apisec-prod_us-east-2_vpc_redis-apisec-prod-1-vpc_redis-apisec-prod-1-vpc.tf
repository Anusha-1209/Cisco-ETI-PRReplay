terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc/us-east-2/redis-apisec-prod-1-vpc.tfstate"
    region = "us-east-2"
  }
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "redis-apisec-prod-1-vpc"
...
module "vpc" {
  ...
  vpc_name                        = "redis-apisec-prod-1"
  cluster_name                    = "redis-apisec-prod-1"
