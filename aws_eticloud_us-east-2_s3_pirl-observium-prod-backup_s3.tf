terraform {
  ...
  backend "s3" {
    ...
    key            = "terraform-state/s3/us-east-2/cisco-eti-pirl-observium-backup-bucket.tfstate"
    region         = "us-east-2"
...
resource "aws_s3_bucket" "b" {
  bucket = "cisco-eti-pirl-observium-backup"
  acl    = "private"
  lifecycle_rule {
    id      = "expire-backup-data-after-7-days"
    enabled = true
