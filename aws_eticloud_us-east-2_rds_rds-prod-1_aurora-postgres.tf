...
terraform {
  backend "s3" {
    ...
    key           = "terraform-state/aurora-postgres/us-east-2/rds-prod-1-rds.tfstate"  #note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
module "rds" {
    ...
    cluster_name = "rds-prod-1"              # the name of the database instance
    database_name = "prod01"
    ...
    CSBApplicationName    = "rds-prod-1-db"
