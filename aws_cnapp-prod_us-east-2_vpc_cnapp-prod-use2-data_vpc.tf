terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc/us-east-2/cnapp-prod-use2-data-vpc.tfstate"
    region = "us-east-2"
  }
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "cnapp-prod-use2-data-vpc"
...
module "vpc" {
  ...
  vpc_name                        = "cnapp-prod-use2-data"
  cluster_name                    = "cnapp-prod-use2-data"
