...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "eks-prod-3-eks-amp-access-management"
...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/aws-eks-prod-3/eks/eks-prod-3-otel-iam.tfstate" # The path should match the pattern terraform_state/<service>/<region>/<name>.tfstate
...
variable "cluster_name" {
  ...
  type        = string
  default     = "eks-prod-3" # The name of the associated EKS cluster. Must be updated.
}

variable "cluster_address" {
  description = "EKS cluster API server endpoint"
  type        = string
  default     = "https://1C56789A6E399D7DBB0A0C32F34B6455.gr7.us-east-2.eks.amazonaws.com"
}
