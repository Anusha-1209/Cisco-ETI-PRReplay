...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/aws/outshift-common-staging/us-east-2/vpc-peering/marvin-staging-compute-to-data.tfstate"
    region = "us-east-2"
  }
}

locals {
  acceptor_vpc_name  = "marvin-stage-use2-1"
  requestor_vpc_name = "marvin-staging-use2-data"
}
...
resource "aws_route" "data-to-compute" {
...
resource "aws_route" "compute-to-data" {
