terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws/rosey-test/eu-cnetral-1/redis/rosey-staging-euc1-1.tfstate"       # UPDATE ME.
...
resource "aws_security_group" "redis_security_group" {
  name   = "rosey-cache-staging-euc1-1-sg"
...
resource "aws_elasticache_replication_group" "rosey-staging-euw1-1" {
  replication_group_id       = "rosey-staging-euc1-1"
  description                = "Redis cluster for rosey-staging-cache-euc1-1 ElastiCache"
  engine                     = "redis"
  engine_version             = "7.1"
