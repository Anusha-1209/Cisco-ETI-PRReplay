...
variable "venture_name" {
  type    = string
  default = "dragonfly" # <-- requires updating
}
...
provider "vault" {
  address   = "https://keeper.cisco.com"
  namespace = "eticloud/apps/dragonfly" # <-- requires updating
  alias     = "venture"
}
...
resource "vault_mount" "kvv2-dragonfly" {
  provider = vault.venture
  path     = "dragonfly-kv"
  type     = "kv"
  options  = { version = "2" }
...
resource "vault_policy" "external-secrets-prod" {
  ...
  policy   = <<EOT
    ...
    path "dragonfly-kv/prod/*" {
      capabilities = ["read", "list"]
    }
    path "dragonfly-kv/data/prod/*" {
      capabilities = ["read", "list"]
    }
...
resource "vault_policy" "external-secrets-prod-dragonfly" {
  provider = vault
  name     = "external-secrets-prod-dragonfly"
  policy   = <<EOT
    path "secret/data/*" {
      capabilities = ["read", "list"]
    }
