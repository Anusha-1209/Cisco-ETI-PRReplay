...
terraform {
  backend "s3" {
    ...
    key           = "terraform-state/eti-ci/us-east-2/s3/matunger05.tfstate" # #note the path here. It should match the pattern terraform_state/<service>/<region>/<name>.tfstate
...
module "s3" {
  ...
  bucket_name = "matunger05"
  CSBApplicationName    = "matunger05"
