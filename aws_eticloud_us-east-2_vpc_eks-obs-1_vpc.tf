...
terraform {
  backend "s3" {
    ...
    key           = "terraform-state/vpc/us-east-2/eks-obs-1.tfstate" # The statefile name should be descriptive and must be unique.
...
module "vpc" {
  ...
  name            = "eks-obs-1" # The name of the VPC should be descriptive and must be unique.
  cidr            = "10.11.0.0/16" # You should use a /16 RFC1918 CIDR.
  region          = var.AWS_INFRA_REGION # Do not change.
  cluster_name    = "eks-obs-1" # The parameter sets tags on subnets for EKS to consume. Please contact the SRE team if you have need more than one EKS cluster in your VPC.
