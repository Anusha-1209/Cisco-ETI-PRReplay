terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws/eticloud/global/rds/global-rds-phoenix-dev-1.tfstate"     
    region  = "us-east-2"                                                                  
  }
}
...
module "rds_primary" {
  ...
  cluster_name        = "global-rds-phoenix-dev-use2-1"
  kms_key_id          = aws_kms_key.primary.arn
  master_username     = "root"
  secret_path         = "secret/dev/infra/aurora-pg/us-east-2/outshift-common-dev/global-rds-phoenix-dev-use2-1"
...
module "rds_secondary" {
  ...
  instances                 = {}
  cluster_name              = "global-rds-phoenix-dev-usw2-1"
  kms_key_id                = aws_kms_replica_key.secondary.arn
  secret_path               = "secret/dev/infra/aurora-pg/us-west-2/outshift-common-dev/global-rds-phoenix-dev-usw2-1"
  db_engine_version         = "15.4"
  db_allowed_cidrs          = [ data.aws_vpc.eks_primary_vpc.cidr_block, data.aws_vpc.eks_secondary_vpc.cidr_block ]
  skip_final_snapshot       = true
