...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/ec2/us-east-2/cisco-research-4.tfstate" # Note the path here. It should match the patten terraform_state/<service>/<region>/<name>.tfstate
...
module "ec2" {
  source                          = "git::https://wwwin-github.cisco.com/eti/sre-tf-module-aws-ec2?ref=1.1.7"
  ec2_name                        = "cisco-research-4" # The name of the instance(s) to be created. Will be appended with a number.
  ...
  tag_application_name            = "cisco-research-4"
