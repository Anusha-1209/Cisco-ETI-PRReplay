...
data "aws_security_group" "eks_sg" {
  ...
  filter {
    name   = "tag:Name"
    values = ["eks-cluster-sg-motf-dev-use2-1-*"]
  }
}
...
resource "aws_secretsmanager_secret" "msk_secret" {
  ...
  description = "SASL/SCRAM secret for msk_motf-dev-use2-1"
}
...
data "vault_generic_secret" "msk_credentials" {
  ...
  path     = "secret/infra/msk/motf-dev-use2-1"
}
...
resource "aws_security_group_rule" "kafka_ingress_eks_motf-dev-use2-1" {
  type              = "ingress"
  from_port         = 9096
  to_port           = 9096
  protocol          = "tcp"
