terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws/eticloud/global/rds/global-rds-platform-prod-1.tfstate"     
    region  = "us-east-2"                                                                  
  }
}
...
module "rds_primary" {
  ...
  cluster_name      = "global-rds-platform-prod-use2-1"
...
module "rds_secondary" {
  ...
  # instances                 = {}
  cluster_name              = "global-rds-platform-prod-usw2-1"
  kms_key_id                = aws_kms_replica_key.secondary.arn
  secret_path               = "secret/prod/infra/aurora-pg/us-west-2/outshift-common-prod/global-rds-platform-prod-usw2-1"
  db_engine_version         = "15.4"
  db_allowed_cidrs          = [ data.aws_vpc.eks_primary_vpc.cidr_block, data.aws_vpc.eks_secondary_vpc.cidr_block ]
