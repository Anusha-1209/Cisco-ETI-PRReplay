...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/vpc-peering/us-east-2/redis-apisec-staging-1-apisec-staging-1/redis-apisec-staging-1-apisec-staging-1.tfstate"
    region = "us-east-2"
  }
}

locals {
  acceptor_vpc_name  = "redis-apisec-staging-1-vpc"
