terraform {
  backend "s3" {
    ...
    key    = "terraform-state/aws-apisec-dev/vpc/us-east-2/apisec-pr-1-vpc.tfstate"
    region = "us-east-2"
  }
}
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "apisec-pr-1-vpc"
...
module "vpc" {
  ...
  private_subnets                 = ["10.7.16.0/20","10.7.32.0/20","10.7.64.0/20"]
  create_secondary_subnets        = false
  vpc_name                        = "apisec-pr-1"
