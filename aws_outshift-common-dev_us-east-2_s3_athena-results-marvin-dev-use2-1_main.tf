...
terraform {
  backend "s3" {
    ...
    key = "terraform-state/aws/outshift-common-dev/us-east-2/s3/athena-results-marvin-dev-use2-1.tfstate"
...
module "s3" {
  ...
  bucket_name = "athena-results-marvin-dev-use2-1" # The name of the S3 bucket.  S3 bucket names are required to be globally unique across all of AWS.
