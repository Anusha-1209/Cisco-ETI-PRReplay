...
terraform {
  backend "s3" {
    ...
    key           = "terraform-state/motific-preview/us-east-2/s3/emr-motf-preview-use2-1.tfstate" # #note the path here. It should match the pattern terraform_state/<service>/<region>/<name>.tfstate
...
provider "aws" {
  ...
  default_tags {
    tags = {
      ApplicationName    = "Motific EMR S3 Bucket"
