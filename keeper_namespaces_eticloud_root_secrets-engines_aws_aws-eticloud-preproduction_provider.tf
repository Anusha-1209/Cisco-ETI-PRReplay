...
data "vault_generic_secret" "aws-eticloud-preproduction-infra-credentials" {
...
provider "aws" {
  alias       = "preprod"
  access_key  = data.vault_generic_secret.aws-eticloud-preproduction-infra-credentials.data["AWS_ACCESS_KEY_ID"]
  secret_key  = data.vault_generic_secret.aws-eticloud-preproduction-infra-credentials.data["AWS_SECRET_ACCESS_KEY"]
  region      = "us-east-2"
  max_retries = 3
}
