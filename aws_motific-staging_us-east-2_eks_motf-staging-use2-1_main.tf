terraform {
  backend "s3" {
    ...
    key = "terraform-state/aws/motific-staging/us-east-2/eks/motf-staging-use2-1.tfstate" # UPDATE ME.
...
module "eks_all_in_one" {
  ...
  cluster_os     = "Ubuntu22"
  min_size       = 10               # EKS node group min size
  max_size       = 15              # EKS node group max size
  desired_size   = 10               # EKS node group desired size
