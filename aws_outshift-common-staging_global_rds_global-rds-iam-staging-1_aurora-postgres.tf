terraform {
  backend "s3" {
    ...
    key     = "terraform-state/aws/eticloud/global/rds/global-rds-iam-staging-1.tfstate"     
    region  = "us-east-2"                                                                  
  }
}
...
module "rds_primary" {
  ...
  cluster_name      = "global-rds-iam-staging-use2-1"
...
module "rds_secondary" {
  ...
  # instances                 = {}
  cluster_name              = "global-rds-iam-staging-usw2-1"
